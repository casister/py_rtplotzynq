...
class CSVProcess(multiprocessing.Process):
    ...
    def __init__(self, filename=None, path=None, timeout=0.5):
        ...
        if filename is None:
            filename = strftime(plot_constants.csv_default_filename, gmtime())
    ...
    def run(self):
        ...
        self._csv = csv.writer(self._file, delimiter=plot_constants.csv_delimiter, quoting=csv.QUOTE_MINIMAL)
    ...
    @staticmethod
    def _create_file(filename, path=None, extension=plot_constants.csv_file_extension):
        """
