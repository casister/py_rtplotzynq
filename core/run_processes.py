...
from constants.constants import PlotConstants, SourceType
from core.ring_buffer import RingBuffer
from processes.Csv import CSVProcess
from processes.parser import ParserProcess
from processes.Serial import SerialProcess
from processes.socket_client import SocketProcess
# from processes.simulator import SimulatorProcess
...
class Worker:
    ...
    def __init__(self,
                 port=None,
                 speed=PlotConstants.serial_default_speed,
                 samples=PlotConstants.argument_default_samples,
                 source=SourceType.serial,
                 export_enabled=False,
                 export_path=PlotConstants.app_export_path):
        """
    ...
    def stop(self):
        ...
        for process in [self._acquisition_process, self._parser_process, self._csv_process]:
            if process is not None and process.is_alive():
                process.stop()
                process.join(PlotConstants.process_join_timeout_ms)
    ...
    def _store_signal_values(self, values):
        ...
        size = len(values)
        if self._lines < size:
            if size > PlotConstants.plot_max_num_signals:
                self._lines = PlotConstants.plot_max_num_signals
            else:
                self._lines = size
    ...
    def reset_buffers(self, samples):
        ...
        :type samples: int.plot_colors
        :return:
        """
        self._data_buffers = []
        for tmp in PlotConstants.plot_signal_colors:
